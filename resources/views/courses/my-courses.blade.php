    ...
    <div class="main-content" id="mainContent">
        <div class="container mx-auto px-4 lg:px-6 py-8">
            ...
            <div class="mb-8">
                ...
                <p class="text-gray-600">Track your academic progress and enrolled courses</p>
            </div>

            <!-- Academic Info Card -->
            @if(isset($academicInfo))
            <div class="bg-gradient-to-r from-blue-500 to-purple-600 rounded-xl shadow-lg p-6 mb-8 text-white">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div class="text-center md:text-left">
                        <h3 class="text-lg font-semibold mb-1">Current Level</h3>
                        <p class="text-2xl font-bold">Level {{ $academicInfo['current_level'] }}</p>
                    </div>
                    <div class="text-center md:text-left">
                        <h3 class="text-lg font-semibold mb-1">Current Term</h3>
                        <p class="text-2xl font-bold">Term {{ $academicInfo['current_term'] }}</p>
                    </div>
                    <div class="text-center md:text-left">
                        <h3 class="text-lg font-semibold mb-1">Admission</h3>
                        <p class="text-lg">{{ $academicInfo['admission_semester'] }} {{ $academicInfo['admission_year'] }}</p>
                    </div>
                    <div class="text-center md:text-left">
                        <h3 class="text-lg font-semibold mb-1">Department</h3>
                        <p class="text-sm">{{ $academicInfo['department'] }}</p>
                    </div>
                </div>
            </div>
            @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div class="bg-white rounded-xl shadow-lg p-6 card-hover">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900">{{ array_sum(array_map('count', $coursesBySemester ?? [])) }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-lg p-6 card-hover">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <p class="text-sm text-gray-600">Semesters</p>
                            <p class="text-2xl font-bold text-gray-900">{{ count($semesterGPAs ?? []) }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Courses by Semester -->
            @if(!empty($coursesBySemester))
                @foreach($coursesBySemester as $semester => $courses)
                <div class="mb-8">
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-2xl font-bold text-gray-900">
                            {{ $semester }}
                            @if(str_contains($semester, 'Current'))
                                <span class="ml-2 px-3 py-1 bg-blue-100 text-blue-800 text-sm font-medium rounded-full">Current</span>
                            @endif
                        </h2>
                        @if(isset($semesterGPAs[$semester]))
                            <div class="text-right">
                                <p class="text-sm text-gray-600">Semester GPA</p>
                                <p class="text-xl font-bold text-gray-900">{{ number_format($semesterGPAs[$semester], 2) }}</p>
                            </div>
                        @endif
                    </div>
                        ...
                        <div class="bg-white rounded-xl shadow-lg overflow-hidden card-hover">
                            <div class="p-6">
                                <div class="flex items-center justify-between mb-4">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        @if($course->is_elective)
                                            <span class="px-2 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800">Elective</span>
                                        @else
                                            <span class="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">Core</span>
                                        @endif
                                        <span class="px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">{{ $course->course_type ?? 'Theory' }}</span>
                                    </div>
                                </div>
                                
                                <h3 class="text-lg font-semibold text-gray-900 mb-2">{{ $course->course_name }}</h3>
                                <p class="text-sm text-gray-600 mb-2">{{ $course->course_code }}</p>
                                
                                <div class="flex items-center justify-between text-sm text-gray-600 mb-4">
                                    <span><i class="fas fa-credit-card mr-1"></i>{{ $course->credits }} Credits</span>
                                    <span><i class="fas fa-building mr-1"></i>{{ $course->department->code ?? 'N/A' }}</span>
                                </div>
                                
                                @if($course->description)
                                <p class="text-sm text-gray-500 mb-4 line-clamp-3">{{ $course->description }}</p>
                                @endif
                                
                                <!-- Enrollment Status -->
                                @php
                                    $isEnrolled = $studentCourses->has($course->id);
                                    $studentCourse = $isEnrolled ? $studentCourses->get($course->id) : null;
                                @endphp
                                ...
                                <div class="mt-4 pt-4 border-t border-gray-200">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <span class="text-sm font-semibold 
                                            @if(in_array($studentCourse->grade, ['A+', 'A', 'A-'])) text-green-600
                                            @elseif(in_array($studentCourse->grade, ['B+', 'B', 'B-'])) text-blue-600
                                            @elseif(in_array($studentCourse->grade, ['C+', 'C', 'C-'])) text-yellow-600
                                            @else text-red-600
                                            @endif">
                                ...
                                <div class="mt-4 flex space-x-2">
                                    <a href="{{ route('courses.show', $course) }}" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-sm py-2 px-4 rounded-lg transition-colors text-center inline-block">
                                        View Details
                                    </a>
                                    @if($course->is_elective)
                                    <button class="flex-1 bg-green-600 hover:bg-green-700 text-white text-sm py-2 px-4 rounded-lg transition-colors">
                                        Enroll
                                    </button>
                                    @endif
                                </div>
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-600">No courses are available for your current academic level and department.</p>
                </div>
            @endif
        </div>
    </div>
