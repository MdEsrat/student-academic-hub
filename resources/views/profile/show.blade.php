        ...
        <div class="container mx-auto px-4 lg:px-6 py-8">
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('profile.edit') }}" class="group bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-8 py-3 rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 hover:shadow-lg flex items-center">
                        <i class="fas fa-edit mr-2 group-hover:animate-pulse"></i>
                        Edit Profile
                        <i class="fas fa-arrow-right ml-2 opacity-0 group-hover:opacity-100 transform translate-x-0 group-hover:translate-x-1 transition-all duration-300"></i>
                    </a>
                </div>
            </div>
                    ...
                    <div class="bg-white rounded-xl shadow-lg p-6 card-hover">
                        <div class="text-center">
                            <div class="relative inline-block">
                                @if($user->profile_picture ?? null)
                                    <img src="{{ asset('storage/' . $user->profile_picture) }}" alt="Profile" class="w-32 h-32 rounded-full mx-auto object-cover border-4 border-blue-100">
                                @else
                                    <div class="w-32 h-32 bg-gray-200 rounded-full mx-auto flex items-center justify-center border-4 border-blue-100">
                            ...
                            <p class="text-gray-600">{{ ucfirst($user->role ?? 'student') }}</p>
                            <p class="text-sm text-gray-500 mt-2">{{ $user->student_id ?? 'DIU-2024-001' }}</p>
                        </div>
                        ...
                        <div class="mt-6 pt-6 border-t border-gray-200">
                            <div class="space-y-3">
                                <div class="flex items-center text-sm">
                                    ...
                                    <span class="ml-3 text-gray-600">{{ $user->email ?? 'test@diu.edu.bd' }}</span>
                                </div>
                                <div class="flex items-center text-sm">
                                    <i class="fas fa-graduation-cap text-gray-400 w-4"></i>
                                    <span class="ml-3 text-gray-600">{{ $user->department->name ?? 'Computer Science' }}</span>
                                </div>
                                <div class="flex items-center text-sm">
                                    <i class="fas fa-calendar text-gray-400 w-4"></i>
                                    <span class="ml-3 text-gray-600">Joined {{ $user->created_at->format('M Y') ?? 'Jan 2024' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="lg:col-span-2">
                    <div class="bg-white rounded-xl shadow-lg p-6 card-hover">
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <!-- Duplicate Edit Profile button removed for cleaner design -->
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div class="p-3 bg-gray-50 rounded-lg border">
                                    <span class="text-gray-900">{{ $user->department->name }} ({{ $user->department->code }})</span>
                                </div>
                            </div>
                            @endif
                            ...
                            @if($user->admission_semester && $user->admission_year)
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Admission Info</label>
                                <div class="p-3 bg-gray-50 rounded-lg border">
                                    <span class="text-gray-900">{{ $user->admission_semester }} {{ $user->admission_year }}</span>
                                </div>
                            </div>
                            @endif

                            @if($user->isStudent() && $user->admission_semester && $user->admission_year)
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Current Academic Status</label>
                                <div class="p-3 bg-blue-50 rounded-lg border border-blue-200">
                                    <span class="text-blue-900 font-medium">Level {{ $user->getCurrentLevel() }} - Term {{ $user->getCurrentTerm() }}</span>
                                </div>
                            </div>
                            @endif

                            @if($user->department && $user->department->semester_type)
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Semester System</label>
                                <div class="p-3 bg-gray-50 rounded-lg border">
                                    <span class="text-gray-900">{{ ucfirst($user->department->semester_type) }}</span>
                                    @if($user->department->semester_type === 'trisemester')
                                        <span class="text-sm text-gray-600 block">Spring, Summer, Fall</span>
                                    @else
                                        <span class="text-sm text-gray-600 block">Spring, Fall</span>
                                    @endif
                                </div>
                            </div>
                            @endif
